...
| MODNet, tract-compatible variant (mobile candidate) | `modnet_photographic_portrait_matting.tract.onnx` (24.7 MB) | Apache-2.0 (same weights, mechanically patched — see below) | derived from the row above via `scripts/patch-modnet-for-tract.py` (not a separate download; run the script to reproduce) | `a09a06eebfbe75e3c7bf0241c4cc26b2709d55ae5193277c6812ce98b3a0447f` (output of the script as committed 2026-08-02) | **wired in as `registry::MODNET_TRACT` on 8 September 2026** — `Modnet::load` picks it under `tract-backend`, and the phone apps (`apps/mobile`) ship it in the bundle rather than downloading it — validated to run under `tract` and match the original `ort` output (mean abs. alpha diff 0.000179 on `testdata/portrait-obama.jpg`), and, since the same day, the runtime path of the iPhone, iPad and Android apps (verified on simulators, `docs/mobile.md`) |
