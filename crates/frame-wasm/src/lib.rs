...
/// Install a panic hook that prints to the browser console instead of
/// failing with an opaque `unreachable`. Call once at start-up.
#[wasm_bindgen(start)]
pub fn start() {
    console_error_panic_hook::set_once();
}

fn err<E: std::fmt::Display>(context: &str) -> impl FnOnce(E) -> JsError + '_ {
...
impl YunetHeads {
    ...
    fn decode(&self, scale: f32) -> Result<Vec<Face>, JsError> {
...
fn matte_from_map(model: &str, map: &[f32], w: u32, h: u32) -> Result<Matte, JsError> {
    match model {
        ...
        other => Err(JsError::new(&format!("unknown matting model {other}"))),
    }
}
...
impl GarmentOption {
    fn to_params(&self) -> Result<garment::GarmentParams, JsError> {
        let style = match self.style.as_str() {
            ...
            other => return Err(JsError::new(&format!("unknown garment style {other}"))),
        };
...
impl Session {
    ...
    pub fn set_detail_matte(&mut self, map: &[f32]) -> Result<(), JsError> {
        ...
        let matte = self
            .matte
            .as_mut()
            .ok_or_else(|| JsError::new("set_matte must run before set_detail_matte"))?;
        let [x, y, w, h] = rect;
                ...
                let backdrop = match choice {
                    ...
                    BackdropOption::Image { blur } => {
                        let rgba = self.backdrop_image.clone().ok_or_else(|| {
                            JsError::new("no backdrop image set — call set_backdrop_image first")
                        })?;
                        Backdrop::Image {
                            rgba,
                            blur_radius: *blur,
    ...
    pub fn solve(
        ...
        let spec = match self.spec.take().filter(|s| s.id == spec_id) {
            ...
            None => frame_compliance::spec::load_all()
                .into_iter()
                .find(|s| s.id == spec_id)
                .ok_or_else(|| JsError::new(&format!("unknown spec {spec_id}")))?,
        };
        let face = self
            .source_face
            .clone()
            .ok_or_else(|| JsError::new("no face detected"))?;
        let prepared = self
            .prepared
            .as_ref()
            .ok_or_else(|| JsError::new("call prepare() before solve()"))?;
    ...
    pub fn output_face_input(&mut self) -> Result<Vec<f32>, JsError> {
        let output = self
            .output
            .as_ref()
            .ok_or_else(|| JsError::new("no output yet"))?;
    ...
    pub fn validate_output(&self, heads: &YunetHeads) -> Result<String, JsError> {
        ...
        let spec = self
            .spec
            .as_ref()
            .ok_or_else(|| JsError::new("no spec selected"))?;
    ...
    fn sheet_layout(&self, sheet: &str) -> Result<(&Frame, frame_sheet::Layout), JsError> {
        ...
        let (sw, sh) = match sheet {
            ...
            other => return Err(JsError::new(&format!("unknown sheet {other}"))),
        };
    ...
    fn output_ref(&self) -> Result<&Frame, JsError> {
        self.output
            .as_ref()
            .ok_or_else(|| JsError::new("no output yet — call solve() first"))
    }
}
