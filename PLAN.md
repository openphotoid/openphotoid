...
| 2026-09-11 | Store step prepared (`store/README.md`): Play upload key + signed AAB, App Store privacy manifest and usage strings from `project.yml`, listings in eight languages within field limits, store-size screenshots per language (`e2e/capture-store.mjs`), review/privacy/rating answers, App Store Connect upload script. Eight-language UI shipped the same day. Fixed along the way: the inter-eye "recommended 120 px" caution fired on every compliant crop because it was unreachable at 600 px within the head band; it now cautions only where the automatic crop could meet it, and the site copy sharing the document is hidden inside the phone apps. Open: the two store accounts. | ✅ Done |
